...
describe Clearance::Generators::InstallGenerator, :generator do
  ...
  def table_does_not_exist(name)
    if ActiveRecord::Base.respond_to?(:data_source_exists?)
      allow(ActiveRecord::Base).to receive(:data_source_exists?).
        with(name).
        and_return(false)
    else
      allow(ActiveRecord::Base).to receive(:table_exists?).
        with(name).
        and_return(false)
    end
  end
